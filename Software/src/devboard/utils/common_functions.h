 ...
 *
 * CRC-16/CCITT (polynomial 0x1021, initial value 0xFFFF, no reflection) over data bytes 2..dlc-1,
 * followed by the low and high byte of the CAN ID, then XORed with a final constant. Almost every
 * ID uses 0x6E17 (verified on every CAN-FD frame of a 22 s EV6 GT recording, all of DLC 8/16/24/32);
 * 0x27A uses 0x3302. The result is stored little-endian in bytes 0-1 of the frame; byte 2 holds an
 * alive counter that increments on every transmission.
 *
 ...
 * @param[in] final_xor Final XOR constant, 0x6E17 unless the ID is known to use another one
 *
 * @return uint16_t Checksum to store in data[0] (low byte) and data[1] (high byte)
 */
extern uint16_t crc16_hyundai_canfd(const uint8_t* data, uint8_t dlc, uint32_t can_id, uint16_t final_xor = 0x6E17)
    __attribute__((pure));
